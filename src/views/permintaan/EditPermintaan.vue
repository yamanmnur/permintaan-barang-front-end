...
export default {
  ...
  methods: {
    postData() {
        ...
        PermintaanService.updateData(this.formData)
        .then(response => {
            this.alertSuccess();
    ...
    remove(index,item) {
        ...
        if(item.id != '') {
            this.formData.deleted_items.push(item);
        } 
        this.formData.items.splice(index, 1);
    },

    getData() {
      PermintaanService.getDetailData(this.$route.params.id)
      .then(response => {
        this.formData = response.data.data
        this.formData.deleted_items = []
      })
